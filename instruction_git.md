...
### Pull request
1.Создаем новую папку, запускаем ее в VScode.
2.На https://github.com/ находим нужный репозиторий.
3.Делаем Fork интересующего нас репозитория
4.Выполняем команду в терминале git clone для нашей версии этого репозитория (нажимаем на кнопку Code, копируем ссылку),например:
 git clone https://github.com/NovikovaCatherina/seminar3_1.git

5.В VScode переходим в клонированный репозиторий командой cd с указанем имени, или открываем в встроенном терминале.
6.Мы создаем ветку с предлагаемыми измененениями remote (не забываем на нее переключиться).
7.Редактируем файлы,добавляем,вносим коммиты.
8.Выполняем команду git push. В случае если в терминале выходит ошибка: The current branch remote has no upstream branch. В данном случае необходимо ввести терминал:

    git push --set-upstream origin remote 

9.Далее в окне на GitHub появляется возможность отправить pull request. Если ее не появилось нажимаем на Contribute.
